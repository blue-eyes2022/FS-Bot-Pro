...
async def private_receive_handler(c: Client, m: Message):
    if MY_PASS:
        ...
        if check_pass== None:
            await m.reply_text("Login first using /login cmd \n don\'t know the pass? request it from @techiesneh_bot")
            return
    ...
    if Var.UPDATES_CHANNEL != "None":
        try:
            ...
            if user.status == "kicked":
                await c.send_message(
                    chat_id=m.chat.id,
                    text="__You Motherfucker, Yᴏᴜ ᴀʀᴇ Bᴀɴɴᴇᴅ ᴛᴏ ᴜsᴇ ᴍᴇ.__\n\n  **Cᴏɴᴛᴀᴄᴛ Dᴇᴠᴇʟᴏᴘᴇʀ @techiesneh_bot ʜᴇ Wɪʟʟ Hᴇʟᴘ Yᴏᴜ**",
                    parse_mode="markdown",
            ...
            await c.send_message(
                chat_id=m.chat.id,
                text="**Sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ Wʀᴏɴɢ. Cᴏɴᴛᴀᴄᴛ ᴍʏ ʙᴏss** @techiesneh_bot",
                parse_mode="markdown",
                disable_web_page_preview=True)
            return
    try:
